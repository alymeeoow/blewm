...
const Header = () => {
  ...
  // Handle clicking on a dropdown item (submenu item)
  const handleDropdownItemClick = (e, servicePath, item) => {
    e.stopPropagation();
    const route = `${servicePath}/${item.toLowerCase().replace(/\s+/g, '-')}`;
    navigate(route);
    handleNavClick();
  };

  // Handle clicking on the main dropdown button
  const handleDropdownButtonClick = (index, service) => {
    if (isMobile) {
      // On mobile, just toggle the dropdown
      handleMobileClick('service', index);
    } else {
      // On desktop, navigate to the service page
              ...
              <div className={`dropdown-menu ${openDropdown === index ? 'open' : ''}`}>
                {service.items.map((item, itemIndex) => (
                  <Link 
                    ...
                    onClick={(e) => handleDropdownItemClick(e, service.path, item)}
